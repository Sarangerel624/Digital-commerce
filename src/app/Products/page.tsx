...
//   let skip = 0;
//              if(currentPage > 1 ) {
//                 skip = pagedev * (currentPage - 1)
//              }
//              let url = `https://dummyjson.com/products?limit=${pagedev}&skip=${skip}`;
//              if (inputValue.trim() !== "") {
//                   url = `https://dummyjson.com/products/search?q=${(inputValue)}&limit=${pagedev}&skip=${skip}`;
//                 }
...
const Page = () => {
    ...
    return(
        <div>
            <Header/>
            <div>
               <div className="w-[400px] m-3 pl-13" > <Input placeholder="Search products..." value={search}  onChange={(e) => setSearch(e.target.value)} /></div>
            </div>
 ...
 <div className="flex gap-2 justify-center mt-10 mb-10">
       {pages.map((page, index) => {
                return (
                   <div key={index} >
                      <Button  onClick={() => setCurrentPage(page)} variant={page === currentPage ? "default" : "secondary"}>{page}</Button>
                   </div>
                )
            })}
 </div>
